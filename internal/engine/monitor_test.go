...
func TestMonitorUsesGlobalLatestSnapshotWithSelectedLiveLogs(t *testing.T) {
	...
	if len(defects) != 0 { t.Fatal(defects) }
	var got monitorDoc
	if err := json.Unmarshal(raw, &got); err != nil { t.Fatal(err) }
	if got.Pipeline != "monitor" || len(got.Tasks) != 2 || got.Tasks[0].Attempt != 2 || len(got.Edges) != 1 { t.Fatalf("snapshot: %+v", got) }
	if got.Tasks[0].Display == nil || got.Tasks[0].Display.Samples[0] != "S01" { t.Fatal("display lost in plan round trip") }
	if len(got.Logs) != 1 || len(got.Logs[0].StdoutTail) != inspectLogTail || !strings.HasSuffix(got.Logs[0].StdoutTail, "live output") { t.Fatalf("live tail: %+v", got.Logs) }
	if snapshotDir(t, dir) != before { t.Fatal("monitor mutated workspace") }
	raw, defects = Inspect(dir, viewMonitor, "", testInstallIdentity())
	if len(defects) != 0 { t.Fatal(defects) }
	if err := json.Unmarshal(raw, &got); err != nil { t.Fatal(err) }
	if len(got.Logs) != 0 { t.Fatal("global refresh read all logs") }
	identity := *testInstallIdentity()
	identity.PipelineImport += "/different"
	if _, defects := Inspect(dir, viewMonitor, "", &identity); len(defects) == 0 { t.Fatal("identity mismatch accepted") }
	if _, defects := Inspect(dir, viewMonitor, "missing", testInstallIdentity()); len(defects) == 0 { t.Fatal("unknown instance accepted") }
	// Derived live pointers must honor the same symlink containment gate.
	if err := os.Remove(filepath.Join(dir, stdout)); err != nil { t.Fatal(err) }
	outside := filepath.Join(t.TempDir(), "outside")
	writeCheckFile(t, outside, "private")
	if err := os.Symlink(outside, filepath.Join(dir, stdout)); err != nil { t.Fatal(err) }
	if _, defects := Inspect(dir, viewMonitor, "a", testInstallIdentity()); len(defects) == 0 { t.Fatal("outside live log accepted") }
}
...
func TestMonitorRejectsMixedControlRevisions(t *testing.T) {
	...
	writeCheckFile(t, filepath.Join(dir, ControlDir, TasksFile), string(data))
	before := snapshotDir(t, dir)
	if _, defects := Inspect(dir, viewMonitor, "", testInstallIdentity()); len(defects) == 0 { t.Fatal("mixed revisions accepted") }
	if snapshotDir(t, dir) != before { t.Fatal("failed read mutated workspace") }
}
