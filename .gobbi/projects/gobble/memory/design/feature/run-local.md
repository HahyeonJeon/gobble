...
- Current: `Run(ctx, *Graph, workspace, cap)` occupies an existing exclusive caller-owned workspace after checks, writes occupancy on `.gobble/run.json`, isolates tasks, copies staged inputs, runs process or Docker tasks, copies published outputs, and persists state. Empty Image is process; non-empty Image is Docker. Docker `--network=none` and UID/GID are isolation conveniences, not a sandbox. Task Env reaches the process job or Docker container. It does not configure the host Docker client. Stage and publish are copy-only and never use hardlinks or symlinks. Input fingerprints cover staged isolate bytes after stage and before Submit. Destination checksums are recorded after successful exclusive publish. Missing digests or hashes are reuse misses. Resume re-evaluates `When`; skip is not sticky. Ordinary execution is bounded only by caller `ctx`. One settlement bound starts after stop or cancellation begins and during Release Reconcile or Poll. It does not fail an uncancelled long task. Known stopped identities persist `incomplete`; an occupying-process identity whose stop cannot be proved before settlement persists `unknown-backend`; occupancy stays. After adapter Wait, Cancel does not signal the stored PID. Unproved PIDs are never signaled. Readiness uses plan wait paths. File waits require a regular file. Group members stage and publish by name. Tree waits require the directory and dest `.gobble-tree.json`. A second Run while occupancy is active is `occupied-workspace`. Release is reconciliation and occupancy close, not deletion. Cap `0` means 1; values above 64 are refused. Isolates remain under `.gobble/tasks/<id>/<instanceSeg>/<shard>/<attempt>/work`. First-horizon exit is not claimed by live canaries.
- Source: `interfaces`
