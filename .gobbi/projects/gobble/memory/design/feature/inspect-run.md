...
- Current: `Inspect(workspace, View, instance, opts ...OccupyOption)` is the shipped read-only library verb. Views are `run`, `instances`, `errors`, `logs`, `timing`, `dag`, `lineage`, `remaining`, `reuse`, and `identity`; there is no events view. `identity` remains readable on install mismatch and reports required, have, match, goos, goarch, and identity mode. Other views require the workspace and current or supplied identity to match and do not mutate control files. Omitted identity binds the current executable for module installs; supplied `WithIdentity` supports the external API and packed runner. Object views return JSON; `instances`, `remaining`, and `reuse` return JSONL. Every record includes `schema_version`. Occupancy liveness comes from flock and lease state, not PID existence. Instances omit Env and may show `skipped`, `incomplete`, `published-unfinalized`, or `unknown-backend`. Remaining is unsuccessful latest work excluding skipped and `published-unfinalized`. A proved-stopped Docker task may retain a RuntimeID for cleanup retry without remaining unknown; unproved Docker remains `unknown-backend`, keeps occupancy active, and blocks Resume. Inspect rejects `--sample` with exit 2. First-horizon installed-path exit is proved for local-pin and packed identities by `go test -tags=live ./tests/install-e2e`. The published agent install is `github.com/HahyeonJeon/gobble@v0.1.0`.
- Source: `interfaces`
