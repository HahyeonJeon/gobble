...
- Current: `Inspect(workspace, View, instance)` is the shipped read-only library verb. `View` constants are `run`, `instances`, `errors`, `logs`, `timing`, `dag`, `lineage`, `remaining`, and `reuse`. Unknown view is `not-found`. `run`, `errors`, `logs`, `timing`, `dag`, and `lineage` return one JSON object. `instances`, `remaining`, and `reuse` return JSONL. Each object or JSONL record includes `schema_version`. Empty instance selects every reserved identity. Logs are pointers plus an optional bounded tail. `run` occupancy reports `active`, diagnostic `host` and `pid`, and `live` from occupancy flock/lease liveness, not PID existence. Instances omit Env. Instances may show `skipped` and `unknown`. Remaining is classified on all latest attempts using recorded dest and input hashes, then instance-filtered for emit. Remaining may skip hashing when recorded hashes exist. Dest content mismatch uses `output-missing`. Remaining omits skipped identities unless Change forces re-evaluation. Remaining and reuse omit `change` until Resume has classified that identity. Inspect does not occupy, create, or rewrite control files. Schema 0 and 1, and PID-only schema-2 occupancy missing a lease, are `unsupported-schema`. There is no events view. Inspect rejects `--sample` (unknown flag, exit 2). Remaining and reuse views passed in `tests/local-e2e` on run-local, WGS, RNA, and Methyl occupy/release/resume surfaces. Shipped `gobble inspect VIEW --workspace DIR [--instance ID]`; library bytes pass through; contract in [architecture/system.md](../architecture/system.md) Interfaces Current.
- Source: `interfaces`
