...
- Current: `Inspect(workspace, View, instance)` is the shipped read-only library verb. Views are `run`, `instances`, `errors`, `logs`, `timing`, `dag`, `lineage`, `remaining`, and `reuse`; there is no events view. `run`, `errors`, `logs`, `timing`, `dag`, and `lineage` return one JSON object. `instances`, `remaining`, and `reuse` return JSONL. Every object or record includes `schema_version`. Inspect refuses mixed `plan.json`, `tasks.json`, and `run.json` snapshot tokens and never occupies, creates, or rewrites control files. Occupancy `live` comes from flock and lease liveness, not PID existence. Host and PID are diagnostic. Instances omit Env and may show `skipped`, `incomplete`, `published-unfinalized`, or `unknown-backend`. Remaining is unsuccessful latest attempts excluding skipped and excluding `published-unfinalized`. Succeeded attempts require recorded destination and staged-input hashes for reuse; missing hashes are misses. A later-process Docker `unknown-backend` keeps occupancy active and blocks Resume. An unproved later-process process identity is classified during Release as `published-unfinalized` or `incomplete`; it does not remain unknown. Inspect rejects `--sample` with exit 2. First-horizon exit remains unclaimed; live packs are canaries, not D4 recovery evidence.
- Source: `interfaces`
