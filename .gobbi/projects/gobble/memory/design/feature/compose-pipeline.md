...
- Current: Authors build a `Pipeline` and call `Compose` for an immutable `Graph`. PathSpec fields are `Dir`, `Lead`, `Name`, `Steps`, and `Ext`. `AddInput` records one PathSpec. `AddInputGroup` originates a named Group pipeline input. A `From` handle must belong to the composed pipeline. A Bind is a single `Spec` or a `Group`, not both. `Group` is named regular-file members on one port. Group `From` requires the same member-name set, including Group `From` a pipeline input. Group `From` a single-file `AddInput` is `missing-input`. Same-task `From` is a cycle. Isolate restage copies workspace From onto dest and records `IO.Source` when those paths differ. A task has `Command` or `Script`, not both. `Env` is author literals. `Merge.branches` is unused author intent; edges come from `Bind.From`. The public surface is unsupported except those PathSpec concepts. First-party dual-entry proofs live in [assets](assets.md), not in this product feature.
- Source: `interfaces`
