...
- Current preview: Pre-1.0 trusted-local `linux/amd64` preview for one exclusive caller-owned workspace, licensed under MIT. Agents use a local-pin Go library and generic command; humans receive a packed runner for one pipeline. Docker is an isolation convenience, not a sandbox. Recovery is Inspect, later-process Release without signaling an unproved process PID, then Resume remaining work. Dest-complete process work persists `published-unfinalized`; incomplete process work reruns. A proved-stopped Docker task may retain a RuntimeID for cleanup retry without wedging occupancy; unproved Docker remains `unknown-backend` and blocks close. Stage and publish are copy-only. First-horizon installed-path exit is proved for the local-pin agent and packed human families by `go test -tags=live ./tests/install-e2e`; no exact-tag release is claimed.
