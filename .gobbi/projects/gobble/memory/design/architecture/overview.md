...
- Current preview: Pre-1.0 trusted-local `linux/amd64` preview for one exclusive caller-owned workspace, licensed under MIT. The published agent install is `github.com/HahyeonJeon/gobble@v0.1.0`. A local path pin remains valid for an unpublished tree. Humans receive a packed runner for one pipeline. Docker is an isolation convenience, not a sandbox. Recovery is Inspect, later-process Release without signaling an unproved process PID, then Resume remaining work. Dest-complete process work persists `published-unfinalized`; incomplete process work reruns. A proved-stopped Docker task may retain a RuntimeID for cleanup retry without wedging occupancy; unproved Docker remains `unknown-backend` and blocks close. Stage and publish are copy-only. First-horizon installed-path exit is proved for the local-pin agent and packed human families by `go test -tags=live ./tests/install-e2e`.
