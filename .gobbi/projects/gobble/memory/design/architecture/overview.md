...
- Current preview: Pre-1.0 trusted-local Linux source preview for one exclusive caller-owned workspace. Docker is an isolation convenience, not a sandbox. Recovery is Inspect, later-process Release without signaling an unproved process PID, then Resume remaining work. Dest-complete process work persists `published-unfinalized`; incomplete process work reruns; Docker `unknown-backend` blocks occupancy close. Stage and publish are copy-only. The license is unset and grants no redistribution. First-horizon exit remains unclaimed.
