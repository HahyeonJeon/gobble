...
- Current: Gobble is a pre-1.0 trusted-local Linux source preview. The supported public contract is `Compose`, `Validate`, `BuildPlan`, `Run`, `Inspect`, `Release`, and `Resume`; Module, Branch, Merge, Scatter, Gather, and When; PathSpec and File, Group, or Tree binds; explicit-path samplesheet parse and load; and structured `Error`, `Defect`, and `DefectCode` values. Other exports are provisional. `LoadSampleSheetFile(path)` is the supported concurrent samplesheet API. The CLI injects `--sample PATH` or process-cwd `samplesheet.csv` before `Pipeline()`. Hidden-path helpers remain provisional process-owned configuration, not goroutine identity. Registered Command, Inputs, Outputs, Params, Env, and Group members are copied, and returned errors are caller-owned. The trusted caller owns one exclusive workspace. Docker `--network=none` and UID/GID are isolation conveniences, not a sandbox. Ordinary Run and Resume execution is bounded only by caller context. One settlement bound begins after stop or cancellation starts and during occupying-process or later-process Release Reconcile or Poll. Recovery is Inspect, then Release, then Resume remaining work. Later-process Release never signals an unproved process PID. File dest path is a present regular file, not a symlink. Group: every named member is a present regular file, not a symlink. Tree: dest directory exists as a directory and dest `.gobble-tree.json` is a present regular file. Directory presence alone is not dest-complete. Do not walk every Tree member for this test. Dest-complete unproved process identities persist `published-unfinalized` and are omitted from remaining; other unproved process identities persist `incomplete` and rerun. Later-process Docker `unknown-backend` keeps occupancy active and blocks Resume. There is no PID adoption, repair verb, public Cancel, Diff, Retry, or Clean. CLI graph verbs require `go` on PATH; consumer `internal/` packages are unsupported. Success stdout is JSON or JSONL only, generated-child user stdout is isolated, failure stdout is empty, and exits are 0, 1, or 2. A spaced valued flag does not consume a dash-token. The license is unset and grants no redistribution. First-horizon exit is not claimed.
- Source: `interfaces`
