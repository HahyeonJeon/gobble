...
- Now: Gobble is a pre-1.0 trusted-local `linux/amd64` preview licensed under MIT. The supported contract is the seven library and CLI verbs; Module, Branch, Merge, Scatter, Gather, and When; PathSpec and File, Group, or Tree binds; explicit-path samplesheet parse and load; and structured errors. Other exports remain provisional. The published agent install is `github.com/HahyeonJeon/gobble@v0.1.0` for the library and `github.com/HahyeonJeon/gobble/cmd/gobble@v0.1.0` for the command. A local path pin remains valid for an unpublished tree. Humans receive one packed runner for one pipeline and need no Go at run time. Graph verbs and pack require `go` on PATH and reject consumer `internal/` packages. Install identity handshakes before `Pipeline()` and before workspace mutation. Run workspaces are caller-owned and exclusive. Stage and publish are copy-only. Recovery is Inspect, later-process Release without an unproved PID signal, then Resume remaining work. Dest-complete process work persists `published-unfinalized`; incomplete process work reruns. A proved-stopped Docker task may retain a RuntimeID for cleanup retry without wedging occupancy; unproved Docker remains `unknown-backend` and blocks close. Hermetic `go test ./...` remains the first check. First-horizon installed-path exit is proved for the local-pin agent and packed human families by `go test -tags=live ./tests/install-e2e`. The module remains `github.com/HahyeonJeon/gobble` at Go 1.26 and SchemaVersion remains 2. The first public tag is immutable repository-root `v0.1.0`, with no `/v0` path, retag, or supported `@latest`. Later patch means no intended Go API, CLI protocol, workspace-schema, or recovery break. Later minor may add features and may declare a pre-1.0 break, naming Go API, CLI, workspace, and recovery effects. `v0.x.y-rc.1` is used only if later requested. Release notes are in `CHANGELOG.md`.
