...
- Now: Product CLI is shipped at `cmd/gobble` on `feat/gobble-cli` `4f5e6b8` plus this Memory commit, from `develop` `13b0664`. Seven product verbs: `compose`, `validate`, `plan`, `run`, `inspect`, `resume`, `release`. Hermetic `go test ./...` includes `cmd/gobble`. No live CLI tests this session. First-horizon exit is not claimed. Module `github.com/HahyeonJeon/gobble` (`go 1.26`) with `internal/path`, `internal/engine`, and `internal/engine/exec`. Same-module package `assets` holds first-party dual-entry proofs (not product tools). Live is `go test -tags=live` and fails closed without Docker. Public library verbs are Compose, Validate, BuildPlan, Run, Inspect, Resume, and Release. PathSpec fields are Dir, Prefix, Base, Suffixes, Ext. A Bind is File, Group, or Tree. `Run(ctx)` and `Resume(ctx)` cancel through context. Document is the only engine payload. SchemaVersion is 2. Scheduler keys `reservedIdentity`. Resume classifies graph-diff Change and does not return `plan-drift`. Occupancy is an owner record on `.gobble/run.json` until `Release`. There is no public Cancel, Diff, Retry, or Clean. Live WGS Inspect+Release+Resume via `assets.WGS()` is shipped. CLI remains required for local-core horizon exit. Pre-bootstrap baseline remains `a2d561fea2846bc1c55213d66a7025dac980f330`.
