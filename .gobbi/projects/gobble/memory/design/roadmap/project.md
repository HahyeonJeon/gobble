...
- Now: Gobble is a pre-1.0 trusted-local Linux source preview. The supported contract is the seven library and CLI verbs; Module, Branch, Merge, Scatter, Gather, and When; PathSpec and File, Group, or Tree binds; explicit-path samplesheet parse and load; and structured errors. Other exports and package `assets` remain provisional proofs. The CLI is at `cmd/gobble`; graph verbs require `go` on PATH and do not support consumer `internal/` packages. `--sample PATH` is injected on compose, validate, plan, run, and resume; omission injects process-cwd `samplesheet.csv`. `LoadSampleSheetFile(path)` is the supported concurrent API. Run workspaces are caller-owned and exclusive. Stage and publish are copy-only. Input fingerprints cover staged isolate bytes; missing hashes miss; Resume re-evaluates `When`. Recovery is Inspect, later-process Release without unproved PID kill, then Resume remaining work. Dest-complete process work persists `published-unfinalized`; incomplete process work reruns; Docker `unknown-backend` blocks occupancy close. The license is unset and grants no redistribution. Hermetic `go test ./...` is the first check and does not use live tags. Live packs remain canaries. First-horizon exit is not claimed and still requires the complete synthetic and small-WGS API and CLI loop with a real Docker task. The module remains `github.com/HahyeonJeon/gobble` at Go 1.26 with `internal/path`, `internal/engine`, and `internal/engine/exec`. SchemaVersion remains 2 with no migration. Plan-time Document expansion of reservedIdentity remains deferred.
