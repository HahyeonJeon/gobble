...
func newRuntime(t *testing.T, config scrnaseq.Config, workspace string, stage bool) *Runtime {
	...
	if err != nil {
		t.Fatalf("Compose scRNA runtime graph: %v", err)
	}
	r := &Runtime{t: t, graph: graph, workspace: workspace, started: make(chan struct{})}
	r.identity, err = gobble.IdentityFromBuildInfo("github.com/HahyeonJeon/gobble/tests/scenarios/internal/scrnaseqscenario")
	if err != nil {
...
// Run invokes the public scRNA Run lifecycle.
func (r *Runtime) Run(ctx context.Context) error {
	return gobble.Run(ctx, r.graph, r.workspace, 12, gobble.WithIdentity(r.identity))
}

// Resume invokes the public scRNA Resume lifecycle with the current graph.
func (r *Runtime) Resume(ctx context.Context) error {
	return gobble.Resume(ctx, r.graph, r.workspace, 12, gobble.WithIdentity(r.identity))
}
...
func (r *Runtime) ResumeWith(ctx context.Context, samples []scrnaseq.Sample, config scrnaseq.Config) error {
	...
	graph, err := gobble.Compose(pipe)
	if err != nil {
		return fmt.Errorf("compose changed scRNA graph: %w", err)
	}
	stageInputs(r.t, r.workspace, samples, config)
	r.graph = graph
	r.docker.setTasks(pc.AllTasks(r.t, pc.MustPlanJSON(r.t, pipe)))
	return gobble.Resume(ctx, graph, r.workspace, 12, gobble.WithIdentity(r.identity))
}
...
func stageInputs(t *testing.T, workspace string, samples []scrnaseq.Sample, config scrnaseq.Config) {
	t.Helper()
	paths := []string{render(t, config.Reference.BarcodeWhitelist.Path)}
	if config.Reference.SimpleafIndex.IsZero() {
		paths = append(paths, render(t, config.Reference.FASTA), render(t, config.Reference.Annotation))
	} else {
		paths = append(paths, render(t, config.Reference.TranscriptToGene))
	}
	for _, sample := range samples {
		for _, run := range sample.Runs {
			paths = append(paths, run.Fastq1, run.Fastq2)
		}
	}
...
func newFakeDocker(tasks []pc.Task, runtime *Runtime) *fakeDocker {
	fake := &fakeDocker{runtime: runtime, modes: make(map[string]fakeMode), consumed: make(map[string][]ConsumedInput), containers: make(map[string]fakeContainer)}
	fake.setTasks(tasks)
	return fake
}
	...
	case "kill":
		if len(args) > 1 {
			if container, ok := f.containers[args[1]]; ok {
				...
				container.exit = 137
				container.log = "simulated scRNA cancellation\n"
...
func (f *fakeDocker) run(args []string, stdout, stderr io.Writer) (int, error) {
	...
	if len(candidates) != 1 {
		_, _ = fmt.Fprintf(stderr, "scRNA fake Docker matched %d tasks", len(candidates))
		return 1, nil
	}
	mount := ""
	for i := 1; i+1 < len(args); i++ {
	...
	f.next++
	id := "scrna-fake-" + strconv.Itoa(f.next)
	container := fakeContainer{mount: mount, image: task.Image}
	switch f.modes[task.ID] {
	case fakeFailInput:
		container.exit = 42
		container.log = "simulated scRNA input rejection for " + task.ID + "\n"
	case fakeBlock:
...
func consumeFile(mount, rel string) (ConsumedInput, error) {
	if rel == "" {
		return ConsumedInput{}, errors.New("scRNA command input has no path")
	}
	path := filepath.Join(mount, filepath.FromSlash(rel))
	f, err := os.Open(path)
	if err != nil {
		return ConsumedInput{}, fmt.Errorf("open scRNA command input %s: %w", rel, err)
	}
	h := sha256.New()
	_, copyErr := io.Copy(h, f)
	closeErr := f.Close()
	if copyErr != nil {
		return ConsumedInput{}, fmt.Errorf("read scRNA command input %s: %w", rel, copyErr)
	}
	if closeErr != nil {
		return ConsumedInput{}, fmt.Errorf("close scRNA command input %s: %w", rel, closeErr)
	}
