...
func DynamicExpansion() *gobble.Pipeline {
	...
	split := p.AddTask(gobble.TaskSpec{
		Name:    "split",
...
func TreeGroup() *gobble.Pipeline {
	...
	tree := p.AddTask(gobble.TaskSpec{
		Name:    "make_tree",
...
func ModuleFanout() *gobble.Pipeline {
	...
	copyProofFile(p.AddModule("left"), "copy", a, gobble.PathSpec{Dir: gobble.Dir("out"), Base: "a", Ext: ".txt"})
	copyProofFile(p.AddModule("right"), "copy", b, gobble.PathSpec{Dir: gobble.Dir("out"), Base: "b", Ext: ".txt"})
	return p
}
...
func SyntheticCohort() *gobble.Pipeline {
	...
	for i := 1; i <= syntheticCohortSize; i++ {
		...
		copyProofFile(p.AddModule(name), "copy", in, out)
	}
	return p
}
