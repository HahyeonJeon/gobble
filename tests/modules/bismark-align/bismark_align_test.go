...
import (
	...
	. "github.com/HahyeonJeon/gobble/assets/modules/bismark-align"
...
func TestBismarkAlignStandaloneComposeBuildPlan(t *testing.T) {
	...
	task := pc.TaskByID(t, raw, "bismark_align")
	...
	pc.AssertIOPath(t, task.Outputs, "bam", "work/bismark-align/aligned_pe.bam")
	pc.AssertIOPath(t, task.Outputs, "report", "work/bismark-align/aligned_PE_report.txt")
	pc.AssertGroupMembers(t, task.Inputs, "index", evidence.Members("work/bismark-genome"))
	pc.AssertNoTaskName(t, pc.AllTasks(t, raw), "index_files")
	for _, rec := range pc.AllTasks(t, raw) {
...
func TestBismarkAlignNestedModule(t *testing.T) {
	...
	idx := bismarkgenome.AddBismarkGenome(mod, hf, bismarkgenome.BismarkGenomeOptions{ExtraArgs: []string{"--verbose"}})
	...
	raw := pc.MustPlanJSON(t, p)
	task := pc.TaskByID(t, raw, "align.bismark_align")
