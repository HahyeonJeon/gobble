...
func TestAddTaskParents(t *testing.T) {
	...
	if got := p.AddTask(spec("root", "root")); got == nil {
	...
	if got := mod.AddTask(spec("modtask", "mod")); got == nil {
		t.Fatalf("AddTask(module) = nil, want task")
	}
	br := p.Branch("br")
	if got := br.AddTask(spec("brtask", "br")); got == nil {
		t.Fatalf("AddTask(branch) = nil, want task")
	}
	mg := p.Merge("mg")
	if got := mg.AddTask(spec("mgtask", "mg")); got == nil {
...
func TestCommandPath(t *testing.T) {
	...
	got, err := modules.CommandPath(spec)
	if err != nil {
	...
	if _, err := modules.CommandPath(gobble.PathSpec{Base: "."}); err == nil {
...
func TestStandalone(t *testing.T) {
	...
	token, err := modules.CommandPath(reads)
	if err != nil {
		t.Fatalf("CommandPath() error = %v", err)
	}

	var seen gobble.Handle
	p := modules.Standalone("wrap", []modules.Input{{Name: "reads", Spec: reads}}, func(parent modules.Parent, hs []gobble.Handle) {
		if len(hs) != 1 {
			t.Fatalf("build handles got %d, want 1", len(hs))
		}
		seen = hs[0]
		cmd := modules.AppendLegacyExtraArgs([]string{"cat", token}, []string{"--flag"})
...
func TestStandaloneGroup(t *testing.T) {
	...
	p := modules.Standalone("wrap-group", []modules.Input{{Name: "idx", Group: idx}}, func(parent modules.Parent, hs []gobble.Handle) {
