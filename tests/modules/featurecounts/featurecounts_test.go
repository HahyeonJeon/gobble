...
func TestATACFeatureCountsRejectsExtraArgsOutsideTypedBAMMembership(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			graph, err := gobble.Compose(atacFeatureCountsPipeline(test.args))
...
func TestATACFeatureCountsAcceptsCompleteOptionsBeforeTypedBAMs(t *testing.T) {
	task := pc.TaskByID(t, pc.MustPlanJSON(t, atacFeatureCountsPipeline([]string{"-Q", "10", "--primary", "--minOverlap=1"})), "featurecounts_atac")
...
func atacFeatureCountsPipeline(extraArgs []string) *gobble.Pipeline {
	...
	_, err := AddATAC(pipeline, bams, saf, ATACOptions{Options: modules.Options{ExtraArgs: extraArgs}})
	if err != nil {
