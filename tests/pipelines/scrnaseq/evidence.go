// Package scrnaseqevidence owns typed access to the scRNA-seq fixture manifest.
...
const (
	CacheDir     = "tests/pipelines/scrnaseq/testdata/cache"
	ManifestPath = "tests/pipelines/scrnaseq/testdata/manifest.json"
	FixtureSheet = "tests/pipelines/scrnaseq/testdata/scrnaseq-samplesheet.csv"
)
...
func Pins() ([]fixture.Pin, error) {
	manifest, err := Manifest()
	if err != nil {
		return nil, fmt.Errorf("decode scRNA-seq fixture manifest: %w", err)
	}
...
func MustPin(name string) fixture.Pin {
	...
	panic("scRNA-seq fixture manifest missing staged pin " + name)
}
