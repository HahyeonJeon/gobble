...
func TestOfficialStagedIdentitiesReachEverySelectedOperation(t *testing.T) {
	...
	staged := scrnaseqevidence.StagedInputs()
	...
	for _, input := range staged {
		if input.Destination == "" || input.Pin.Name == "" || byPath[input.Destination] != "" || byName[input.Pin.Name] {
			t.Fatalf("invalid or duplicate scRNA staging record: %+v", input)
		}
		byPath[input.Destination] = input.Pin.Name
		byName[input.Pin.Name] = true
	}
	if len(byPath) != len(scrnaseqevidence.MustPins()) || len(byPath) != 9 {
		t.Fatalf("scRNA staging plan has %d identities, want all 9 manifest pins", len(byPath))
	}
	...
	for _, endpoint := range []string{"multiqc", "cohort.h5ad_concat"} {
...
func decodeOperationPlan(t *testing.T, raw []byte) operationPlan {
	...
	if err := json.Unmarshal(raw, &plan); err != nil {
		t.Fatalf("decode scRNA operation plan: %v", err)
	}
	return plan
}
