// Package atacseqevidence owns typed access to the ATAC-seq fixture manifest.
...
const (
	CacheDir     = "tests/pipelines/atacseq/testdata/cache"
	ManifestPath = "tests/pipelines/atacseq/testdata/manifest.json"
	FixtureSheet = "tests/pipelines/atacseq/testdata/atacseq-samplesheet.csv"
)
...
func Pins() ([]fixture.Pin, error) {
	manifest, err := Manifest()
	if err != nil {
		return nil, fmt.Errorf("decode ATAC-seq fixture manifest: %w", err)
	}
...
func MustPin(name string) fixture.Pin {
	...
	panic("ATAC-seq fixture manifest missing staged pin " + name)
}
