...
func TestDESeq2StandaloneComposeBuildPlan(t *testing.T) {
	...
	script, ok := flagValue(task.Command, "-e")
	if !ok {
	...
	assertIOPath(t, task.Inputs, "counts", "work/deseq2/counts.csv")
	assertIOPath(t, task.Outputs, "results", "work/deseq2/results.csv")
}
...
func TestDESeq2NestedModule(t *testing.T) {
	...
	raw := mustPlanJSON(t, p)
	task := planTask(t, raw, "deg.deseq2")
	if task.Name != "deseq2" || task.Module != "deg" {
		t.Fatalf("nested task = %+v, want name deseq2 module deg", task)
	}
	if !containsAll(task.Command, "--quiet", "ctrl", "treat", "work/deseq2/results.csv") {
