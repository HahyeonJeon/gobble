...
func TestScatterGatherRunAndPlan(t *testing.T) {
	...
	raw := mustPlanJSON(t, ScatterGather())
	tasks := planAllTasks(t, raw)
	if len(tasks) != 2 {
		t.Fatalf("plan tasks got %d, want 2 authored", len(tasks))
	}
	mustHaveTaskID(t, tasks, "each.copy")
	mustHaveTaskID(t, tasks, "all.join")
...
func TestDynamicExpansionRunAndPlan(t *testing.T) {
	dir := t.TempDir()
	raw := mustPlanJSON(t, DynamicExpansion())
	tasks := planAllTasks(t, raw)
	mustHaveTaskID(t, tasks, "split")
	mustHaveTaskID(t, tasks, "each.copy")
	mustHaveTaskID(t, tasks, "all.join")
...
func TestSyntheticCohortRun(t *testing.T) {
	dir := t.TempDir()
	raw := mustPlanJSON(t, SyntheticCohort())
	tasks := planAllTasks(t, raw)
	...
	for i := 1; i <= syntheticCohortSize; i++ {
		...
		mustHaveTaskID(t, tasks, name+".copy")
		if planTask(t, raw, name+".copy").Image != "" {
...
func assertNoTaskID(t *testing.T, tasks []planTaskRec, id string) {
	t.Helper()
