// Package fastqc owns the graph-stable FastQC command module.
package fastqc
...
func AddFastQC(parent modules.Parent, reads gobble.Handle, opts FastQCOptions) FastQCPorts {
...
func FastQCPipeline(reads gobble.PathSpec, opts FastQCOptions) *gobble.Pipeline {
	return modules.Standalone("fastqc", []modules.Input{{Name: "reads", Spec: reads}}, func(parent modules.Parent, hs []gobble.Handle) {
...
func addFastQC(parent modules.Parent, reads gobble.Handle, opts FastQCOptions) FastQCPorts {
	...
	cmd = append(cmd, modules.MustCommandPath(reads.Spec()))
