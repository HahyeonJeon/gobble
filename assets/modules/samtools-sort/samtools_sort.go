// Package samtoolssort owns the graph-stable samtools sort command module.
...
// samtoolsImage is the graph-stable WGS checkpoint image.
...
func AddSamtoolsSort(parent modules.Parent, sam gobble.Handle, opts SamtoolsSortOptions) SamtoolsSortPorts {
...
func SamtoolsSortPipeline(sam gobble.PathSpec, opts SamtoolsSortOptions) *gobble.Pipeline {
	return modules.Standalone("samtools-sort", []modules.Input{{Name: "sam", Spec: sam}}, func(parent modules.Parent, hs []gobble.Handle) {
...
func addSamtoolsSort(parent modules.Parent, sam gobble.Handle, opts SamtoolsSortOptions) SamtoolsSortPorts {
	...
	cmd = append(cmd, "-o", modules.MustCommandPath(bamSpec))
	cmd = modules.AppendLegacyExtraArgs(cmd, opts.ExtraArgs)
	cmd = append(cmd, modules.MustCommandPath(sam.Spec()))
