// Package multiqc owns the graph-stable MultiQC command module.
package multiqc
...
// call AddInput.
func AddMultiQC(parent modules.Parent, reports []gobble.Handle, opts MultiQCOptions) MultiQCPorts {
...
func MultiQCPipeline(reports []gobble.PathSpec, opts MultiQCOptions) *gobble.Pipeline {
	...
	for i, spec := range reports {
		inputs[i] = modules.Input{Name: multiqcReportBind(i), Spec: spec}
	}
	return modules.Standalone("multiqc", inputs, func(parent modules.Parent, hs []gobble.Handle) {
...
func addMultiQC(parent modules.Parent, reports []gobble.Handle, opts MultiQCOptions) MultiQCPorts {
	...
	for i, report := range reports {
		...
		cmd = append(cmd, modules.MustCommandPath(report.Spec()))
	}
