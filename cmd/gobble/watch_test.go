...
func TestWatchInvocationAndRedirectedOutput(t *testing.T) {
	for _, args := range [][]string{{"watch", "--workspace", "run"}, {"--workspace=run", "watch"}} {
		req, err := parse(args)
		if err != nil || req.command != "watch" || req.workspace != "run" {
	...
	if code := runWatch(&request{command: "watch", workspace: "run"}, &stderr); code == 0 || !strings.Contains(stderr.String(), "terminal") {
		t.Fatalf("redirected watch: %d %s", code, &stderr)
	}
}
