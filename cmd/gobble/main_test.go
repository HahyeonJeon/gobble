...
func TestInvocationFailures(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "workspace cannot consume sample flag", args: []string{"run", "--workspace", "--sample", "x.csv"}, op: "run"},
		{name: "sample cannot consume workspace flag", args: []string{"run", "--sample", "--workspace", dir}, op: "run"},
		{name: "cap cannot consume workspace flag", args: []string{"run", "--cap", "--workspace", dir}, op: "run"},
		{name: "cap cannot consume negative token", args: []string{"run", "--workspace", dir, "--cap", "-1"}, op: "run"},
