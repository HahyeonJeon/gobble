...
type cell struct { text string; role int }
type canvas struct { cells [][]cell; width, height, offset int }
...
func newCanvas(width, height, offset int) *canvas {
	...
	for y := range c.cells {
		...
		for x := range c.cells[y] { c.cells[y][x].text=" " }
	}
	return c
}
...
func (c *canvas) put(x,y int, text string, role int) {
	y -= c.offset
	if x>=0 && x<c.width && y>=0 && y<c.height { c.cells[y][x]=cell{text:text,role:role} }
}

func (c *canvas) text(x,y int, text string, role, limit int) {
	used:=0
	for _, r := range oneLine(text) {
		w:=lipgloss.Width(string(r))
		if used+w>limit { break }
		if w==0 { continue }
		c.put(x+used,y,string(r),role)
		for j:=1;j<w;j++ { c.put(x+used+j,y,"",role) }
		used+=w
	}
}
...
func (c *canvas) stroke(x,y int, glyph string, role int) {
	row:=y-c.offset
	if x<0 || x>=c.width || row<0 || row>=c.height { return }
	old:=c.cells[row][x]
	if old.text!=" " && old.text!=glyph { glyph="┼" }
	// A highlighted edge remains highlighted when another edge crosses it.
	if old.role==2 { role=2 }
...
func (c *canvas) render(t theme) []string {
	...
	for y,row:=range c.cells {
		...
		for _, cell:=range row {
			if cell.role!=role {flush();role=cell.role}
			span.WriteString(cell.text)
		}
		flush();lines[y]=out.String()
	}
	return lines
}

func nodeStatus(c monitor.Counts) string {
	if c.Total == 0 && c.Templates == 0 { return "no owned work" }
	parts:=[]string{}
	for _, item:=range []struct{n int;label string}{{c.Running,"run"},{c.Failed,"fail"},{c.Blocked,"blocked"},{c.Pending,"wait"},{c.Skipped,"skip"},{c.Incomplete,"incomplete"},{c.Unknown,"unknown"},{c.Unfinalized,"unfinalized"}} {
		if item.n>0 {parts=append(parts,fmt.Sprintf("%d %s",item.n,item.label))}
	}
	if c.Unexpanded>0 {parts=append(parts,"expanding")}
	if len(parts)==0 {return "complete"}
...
func (m *model) revealStage() {
	...
	if r.y<m.graphOffset {m.graphOffset=r.y}
	if r.y+r.h>m.graphOffset+h {m.graphOffset=max(0,r.y+r.h-h)}
...
func (m *model) graph(width,height int) []string {
	...
	for i,edge:=range m.data.Edges {
		...
		role:=1
		if edge.From==m.stage || edge.To==m.stage {role=2}
		if x1==x2 {
	...
	for _,stage:=range m.data.Stages {
		...
		role:=1
		if counts.Failed>0 {role=4} else if counts.Attention()>0 {role=5} else if counts.Running>0 {role=2} else if counts.Successful() {role=3}
		if stage.ID==m.stage {role=2}
		for y:=r.y;y<r.y+r.h;y++ {for x:=r.x;x<r.x+r.w;x++ {c.put(x,y," ",0)}}
		c.hline(r.x+1,r.x+r.w-2,r.y,role);c.hline(r.x+1,r.x+r.w-2,r.y+r.h-1,role)
		c.vline(r.x,r.y+1,r.y+r.h-2,role);c.vline(r.x+r.w-1,r.y+1,r.y+r.h-2,role)
		c.put(r.x,r.y,"┌",role);c.put(r.x+r.w-1,r.y,"┐",role)
		c.put(r.x,r.y+r.h-1,"└",role);c.put(r.x+r.w-1,r.y+r.h-1,"┘",role)
		name:=" "+oneLine(stage.Name)+" "
		if stage.ID==m.stage {name=" ▸ "+oneLine(stage.Name)+" "}
		c.text(r.x+1,r.y,fit(name,r.w-3),role,r.w-3)
		first:=fmt.Sprintf("%d / %d succeeded",counts.Succeeded,counts.Total)
		if m.sample!="" && stage.Scope!="sample" {first="Context: "+first}
		if counts.Total==0 && counts.Templates==0 {first="Outside sample scope"}
